                        ...
                        <div class="d-flex flex-column flex-lg-row mb-17">
                            ...
                            <div class="flex-lg-row-fluid me-0 me-lg-20">
                                ...
                                <form class="form mb-15 fv-plugins-bootstrap5 fv-plugins-framework" method="POST"
                                    ...
                                    id="kt_careers_form" action="{{ route('invoice.store') }}"
...
<label class="fs-5 fw-semibold mb-2">Proforma</label>
<select class="form-control form-control-solid select"  name="proforma_id" onchange="getProforma(this)" >
                                            <option value="">Select Proforma</option>

                                            @foreach($proformas as $p)
                                            @php 
                                            $chk = checkInvoice($p->id);
                                            @endphp
                                            @if($chk)
                                            @continue
                                            @endif
                                            <option value="{{$p->id}}" {{Request::get('performa_id')==$p->id ? 'selected': ''}}>{{$p->invoice_number.' '.$p->party->name}}</option>
                                            @endforeach
                                            </select>
                                ...
                                @endphp
                                <tr>
                                <td><select class="w-50px job_id"  name="invoice[{{$i}}][job_id]" {{$required}}>
                                            @foreach($jobs as $job)
                                            <option value="{{$job->id}}" {{$item->job_id==$job->id ? 'selected' : ''}}>{{$job->name}}</option>
                                            @endforeach
                                            </select></td>
                                    <td><select class="w-50px" name="invoice[{{$i}}][category_id]" onchange="getService(this,{{$item->service_id}})" {{$required}}>
                                    ...
                                    <td><select class="w-50px" name="invoice[{{$i}}][service_id]" onchange="serviceDetail(this)" {{$required}}><option value="">Service</option>
                                   
                                </select></td>
                                    <td>
          <div style="display:table-cell;vertical-align:middle;padding-left: 2px;">
          <input class="w-100px date1" name="invoice[{{$i}}][from]" placeholder="From date" value="{{$item->from}}"></div>

          <div style="text-align:right;display:table-cell;vertical-align:middle;padding-left: 10px;">
          <input class="w-100px date1" name="invoice[{{$i}}][to]" placeholder="To date" value="{{$item->to}}"></div>
      </td>
                                    <td><input type="text" name="invoice[{{$i}}][sac]" class="w-50px" value="{{$item->sac}}" {{$required}}></td>
                                    <td><input type="text" name="invoice[{{$i}}][gst]" class="w-50px" value="{{($proforma->billing_id==1 || $proforma->billing_id==2 || $proforma->billing_id==9 || $proforma->billing_id==10) ? $item->gst : 0 }}" onkeydown="getGst()" {{$required}}></td>
                                    <td><input type="text" name="invoice[{{$i}}][rate]" class="w-100px" value="{{$item->rate}}" onkeydown="getRate()" {{$required}}></td>
                                    <td><textarea row="3" name="invoice[{{$i}}][remarks]" class="w-100px">{{$item->remarks}}</textarea></td>
                                    <td><textarea row="3" name="invoice[{{$i}}][Internal_job]" class="w-100px">{{$item->remarks}}</textarea></td>
                                    <td>
                                    <select name="invoice[{{$i}}][type]" class="w-50px" onchange="checkJob(this)" {{$required}}>
                                    ...
                                    <select class="w-100px select2" name="invoice[{{$i}}][job_exe_id][]" multiple="multiple" >
                                        @foreach($job_exe as $exe)
                                ...
                                @endphp
                                <tr>
                                    <td><select class="w-50px select job_id"  name="invoice[{{$i}}][job_id]" {{$required}}>
                                            <option value="">Select</option>
                                    ...
                                    <td><select class="w-50px select" name="invoice[{{$i}}][category_id]" onchange="getService(this,0)" {{$required}}>
                                        <option value="">Category</option>
                                        @foreach($categories as $cat)
          ...
          <input class="w-100px date1" name="invoice[{{$i}}][to]" placeholder="To date"></div>
                                    ...
                                    <td><textarea row="3" name="invoice[{{$i}}][Internal_job]" class="w-100px"></textarea></td>
...
<script>
    var bill_id = "{{ old('billing_id') }}" 
    var party = "{{ old('billing_id') }}" 
