...
<div class="app-main flex-column flex-row-fluid" id="kt_app_main">
                    ...
                    <div class="card-header border-0 pt-5">
                        ...
                        <div class="card-toolbar">
                            ...
                            <a href="{{ route('invoice.create') }}"
                                ...
                                <a href="{{url('invoice/download-excel?')}}keyword={{ !empty(Request::get('keyword')) ? Request::get('keyword') : '' }}&status={{ !empty(Request::get('status')) ? Request::get('status') : '' }}&with_proforma={{ !empty(Request::get('with_proforma')) ? Request::get('with_proforma') : '' }}&invoice_date={{ !empty(Request::get('invoice_date')) ? Request::get('invoice_date') : '' }}&billing={{ !empty(Request::get('billing')) ? Request::get('billing') : '' }}&party={{ !empty(Request::get('party')) ? Request::get('party') : '' }}" class=" mr-3 btn btn-icon btn-success" title="download">
    <i class="fa fa-download fa-lg"></i>
           ...
           <input type="text" name="keyword" class="form-control" value="{{ !empty(Request::get('keyword')) ? Request::get('keyword') : '' }}">
            ...
            <div class="col-md-3 fv-row fv-plugins-icon-container">
            <label class="required fs-6 fw-semibold mb-2">Invoice generated</label>
            <select class="form-control" name="with_proforma">
            <option value="">Select Status</option>
            <option value="1" {{ !empty(Request::get('with_proforma')) && (Request::get('with_proforma')==1) ? 'selected' : '' }}>With proforma invoice</option>
            <option value="2" {{ !empty(Request::get('with_proforma')) && (Request::get('with_proforma')==2) ? 'selected' : '' }}>Without proforma invoice</option>
            </select>
            </div>
            <div class="col-md-3 fv-row fv-plugins-icon-container">
            <label class="required fs-6 fw-semibold mb-2">Date Picker</label>
           <input type="text" name="invoice_date" class="form-control date-range" value="{{ !empty(Request::get('invoice_date')) ? Request::get('invoice_date') : '' }}">
            </div>
            <div class="col-md-3 fv-row fv-plugins-icon-container">
                            ...
                            <table class="table align-middle gs-0 gy-4">
                                ...
                                <thead>
                                    <tr class="fw-bold text-muted bg-light">
                                        ...
                                        <th class="min-w-10px">Billing</th>
                                ...
                                <tbody>
                                    @foreach($data as $val)
                                        <tr>
                                            ...
                                            @endphp
                                            <td>{{$status}}</td>
                                            <td><select class="form-control" name="status" onchange="changeStatus(this,{{$val->id}},'Invoice')">
                                            ...
                                            <div class="d-flex flex-shrink-0">
																<a href="javascript:" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" onclick="viewInvoice({{$val->id}})">
                                                                ...
                                                                <a href="{{ route('invoice.edit', $val->id) }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
																	<!--begin::Svg Icon | path: icons/duotune/art/art005.svg-->
																	<span class="svg-icon svg-icon-3">
                                                                ...
                                                                <form action="{{ route('invoice.destroy', $val->id) }}" method="POST" id="delete-{{$val->id}}">
  @csrf
  @method("DELETE")
