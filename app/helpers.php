...
if (!function_exists('preloadCss')) {
    ...
    if (!function_exists('getJobExecution')) {
        ...
        function getJobExecution($array)
    
        {
            $names = [];
            $result = JobExecution::whereIn('id', $array)->select('name')->get()->toArray();
            if(!empty($result))
...
if (!function_exists('checkInvoice')) {
        ...
        {
            if (Invoice::where('proforma_id',$id)->exists()) {
                  return true;
            }
            else
            {
